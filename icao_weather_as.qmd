...
# url <- "https://en.wikipedia.org/wiki/List_of_Alaska_Airlines_destinations"
# 
# # Read the webpage
# webpage <- read_html(url)
# 
# # Extract the table with ICAO codes
# icao_codes <- webpage %>%
#   html_nodes("table.wikitable") %>%
#   html_table(fill = TRUE) %>% 
#   .[[2]] %>%    # Extract the first table
#   as.data.frame() %>% 
#   filter(Ended %in% c("present", "Present")) %>% 
#   select(City, Country, ICAO)
# 

airports_dim <- read_csv(here("AIRPORT_DIM.csv")) %>% 
